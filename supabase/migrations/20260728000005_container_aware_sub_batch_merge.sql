-- ============================================================================
-- CONTAINER-AWARE SUB-BATCH MERGES
-- ============================================================================
-- Merging physically stored portions creates a new destination portion.
-- Sources are zeroed through adjustments so their storage and test history is
-- retained instead of being deleted by cascading foreign keys.

DROP FUNCTION IF EXISTS public.fn_merge_sub_batches(UUID[], TEXT);
...
BEGIN
  ...
  IF p_location_id IS NULL THEN
    RAISE EXCEPTION 'A destination storage location is required';
  END IF;

  -- Lock in a stable order so concurrent operations over overlapping sources
  -- cannot deadlock by acquiring the same rows in a different order.
  PERFORM sb.id
...
COMMENT ON FUNCTION public.fn_merge_sub_batches(
  ...
  'Merges stored sub-batches into a new destination while preserving source rows, tests, weight history, and storage history.';
